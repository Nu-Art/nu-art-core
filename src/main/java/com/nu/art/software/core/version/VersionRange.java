...
package com.nu.art.software.core.version;
...
public final class VersionRange {
	...
	private String[] validateVersion(String initialVersion)
		...
		if (version.length != 3) {
			throw new BadVersionFormat("Version syntax should be: {x,y-z}.{*}.{*}, and not: " + initialVersion);
		}
		for (int i = 0; i < version.length; i++) {
			if (VersionRange.IndexAnalyzer.instanceCount(initialVersion) == 0) {
				throw new BadVersionFormat(VersionRange.VersionParameter[i] + " index, does not match regex: " + VersionRange.VersionIndexRegex + ". Syntax: {x,y-z}.{*}.{*}");
			}
